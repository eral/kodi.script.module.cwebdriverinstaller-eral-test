...
install() {
	...
	wget https://bootstrap.pypa.io/pip/get-pip.py -O "${TEMP_DIR}/get-pip.py"
	mkdir "${TEMP_DIR}/pip"
	python3 "${TEMP_DIR}/get-pip.py" --no-cache-dir --upgrade --target "${TEMP_DIR}/pip" --log "${TEMP_DIR}/install-pip.log"
	export PYTHONPATH=$PYTHONPATH:${TEMP_DIR}/pip

	mkdir -p "$1"
	export INSTALL_DIR=$(readlink -f "$1")

	python3 -m pip install pyppeteer --no-cache-dir --upgrade --target "${INSTALL_DIR}" --log "${TEMP_DIR}/install-pyppeteer.log"
}
